...
public sealed partial class GunSystem
{
    ...
    private void OnChamberMagazineAppearance(EntityUid uid, ChamberMagazineAmmoProviderComponent component, ref AppearanceChangeEvent args)
    {
        ...
        if (TryComp<MagazineVisualsComponent>(uid, out var visuals) && visuals.FullSprite)
        {
            var state = "base";
            if (!boltClosed)
                state = "bolt-open";
            else if (Appearance.TryGetData(uid, AmmoVisuals.MagLoaded, out bool loaded) && loaded)
            {
                Appearance.TryGetData(uid, AmmoVisuals.AmmoCount, out int count);
                state = $"{visuals.MagState}-{(count > 0 ? 1 : 0)}";
            }

            if (visuals.FoldedStatePrefix != null &&
                Appearance.TryGetData(uid, FoldableSystem.FoldedVisuals.State, out bool folded) && folded)
                state = visuals.FoldedStatePrefix + state;

            _sprite.LayerSetRsiState((uid, args.Sprite), GunVisualLayers.Base, state);
            return;
        }
