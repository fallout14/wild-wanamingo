...
public sealed partial class GunSystem
{
    ...
    {
        // Internal tube providers have no status control of their own. Refresh their gun,
        // including when a server correction changes the tube's predicted ammunition count.
        if (Containers.TryGetContainingContainer(uid, out var container) &&
            HasComp<Content.Shared._Misfits.Weapons.TwinTube.TwinTubeAmmoProviderComponent>(container.Owner))
        {
            UpdateAmmoCount(container.Owner, prediction);
            return;
        }
