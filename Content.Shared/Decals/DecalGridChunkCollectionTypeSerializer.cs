            ...
            ISerializationManager.InstantiationDelegate<DecalGridChunkCollection>? _ = default)
        {
            ...
            if (version > 1)
            {
                ...
                foreach (var dNode in nodes)
                {
                    ...
                    {
                        var dUid = serializationManager.Read<uint>(new ValueDataNode(decalUidNode), hookCtx, context);
            ...
            ISerializationContext? context = null)
        {
            ...
            foreach (var data in lookupNodes)
            {
                ...
                foreach (var uid in uids)
                {
                    ...
                    // Inline coordinates
                    decks.Add(((ValueDataNode) serializationManager.WriteValue(uid, alwaysWrite, context)).Value, serializationManager.WriteValue(decal.Coordinates, alwaysWrite, context));
                }
