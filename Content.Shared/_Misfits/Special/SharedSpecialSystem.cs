...
public sealed class SharedSpecialSystem : EntitySystem
{
    ...
    /// <summary>
    /// Scales weapon scope acquisition time using effective Perception, including temporary modifiers.
    /// </summary>
    public float GetPerceptionScopeDelayMultiplier(EntityUid uid)
    {
        var tuning = GetTuning();
        return GetPerceptionScopeDelayMultiplier(GetEffective(uid, SpecialStat.Perception),
            tuning.PerceptionScopeDelayMultiplierLow, tuning.PerceptionScopeDelayMultiplierHigh);
    }

    public static float GetPerceptionScopeDelayMultiplier(int perception, float low, float high)
    {
        perception = Math.Clamp(perception, SpecialProfile.Minimum, SpecialProfile.Maximum);
        // Preserve the scope's authored delay at 5; interpolate separately on either side.
        var multiplier = perception <= SpecialProfile.DefaultValue
            ? low + (1f - low) * (perception - SpecialProfile.Minimum) /
                (SpecialProfile.DefaultValue - SpecialProfile.Minimum)
            : 1f + (high - 1f) * (perception - SpecialProfile.DefaultValue) /
