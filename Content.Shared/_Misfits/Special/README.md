...
- Weapon scope acquisition uses effective Perception: delay scales linearly from `perceptionScopeDelayMultiplierLow` (3x at PER 1) to 1x at PER 5, then to `perceptionScopeDelayMultiplierHigh` (0.1x at PER 10). Built-in and attached weapon scopes retain their per-zoom base delays; binoculars are unchanged.
